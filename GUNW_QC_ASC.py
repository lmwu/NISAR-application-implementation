...
# 1. 取得第一個 HDF5 檔案
files = sorted(glob.glob("./data_gunw/ASC/*.h5") + glob.glob("./data_gunw/*.nc"))
if not files:
    raise FileNotFoundError("./data_gunw/ASC 目錄內找不到 .h5 或 .nc 檔案！")
...
plt.tight_layout()
plt.savefig("./mailiao_gunw_qc_ASC.png", dpi=300)
plt.show()
print("QC 圖表已修復 orientation 與坐標系，並儲存至 ./mailiao_gunw_qc_ASC.png")
